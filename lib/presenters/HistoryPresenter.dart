  ...
  try {
    ...
    if (hasNetwork) {
      ...
      if (response.statusCode == 200) {
        ...
        List<HistorySearch> localSearchHistory = await getSearchHistoryFromLocal(userId);

        if (localSearchHistory.isNotEmpty) {
          print('Local search history loaded successfully: $localSearchHistory');
          return localSearchHistory;
        } else {
          throw Exception('Failed to fetch search history from API and no local data available');
        }
      }
    } else {
...
static Future<List<HistorySearch>> getSearchHistoryFromLocal(int userId) async {
  ...
  if (searchHistoryJsonList != null) {
    ...
    return searchHistoryJsonList.map((jsonString) => HistorySearch.fromJson(json.decode(jsonString))).toList();
  } else {
...
static Future<void> saveSearchHistory(List<HistorySearch> searchHistory) async {
  ...
  final searchHistoryJsonList = searchHistory.map((history) => json.encode(history.toJson())).toList();
...
static Future<void> sendSearchHistoryToServer(String content, int userId) async {
  try {
    bool hasNetwork = await hasNetworkConnection();
...
static Future<void> saveSearchHistoryToLocal(String content, int userId) async {
  try {
...
static Future<int> getNextSearchHistoryId(List<HistorySearch> searchHistory) async {
  ...
  static Future<void> sendDeleteRequestToServer(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Xử lý lỗi và trả về phản hồi thích hợp
        print('Error deleting search history from server: ${response.statusCode}');
      }
    } catch (e) {
